...
local Toggle = {
    ...
    NoRecoil = false, FastShoot = false, InfiniteAmmo = false
}
...
main.Size = UDim2.new(0, math.min(420, screenSize.X - 20), 0, math.min(520, screenSize.Y - 40))
...
leftBar.Size = UDim2.new(0, 80, 1, 0)
...
leftList.CanvasSize = UDim2.new(0, 0, 0, 300)
...
for i, name in ipairs(categories) do
    ...
    btn.Position = UDim2.new(0, 3, 0, (i-1) * 36 + 4)
    btn.Text = name
...
rightArea.Size = UDim2.new(1, -80, 1, 0)
rightArea.Position = UDim2.new(0, 80, 0, 0)
...
local function CreateSlider(parent, name, y, min, max, default, cb)
    ...
    d.Position = UDim2.new((default-min)/(max-min), -7, 0, -5)
...
y = y + 42
CreateToggle(panel2, "无限跳跃", y, "空中无限跳", function(s) 
...
y = y + 24
CreateToggle(panel4, "无限弹药", y, "子弹用不完", function(s) Toggle.InfiniteAmmo = s end)
y = y + 42
CreateToggle(panel4, "无后座力", y, "武器无后座", function(s) Toggle.NoRecoil = s end)
y = y + 42
CreateToggle(panel4, "快速射击", y, "射速提升", function(s) Toggle.FastShoot = s end)
y = y + 46
...
hotkeyInfo.Size = UDim2.new(1, -10, 0, 80)
hotkeyInfo.Position = UDim2.new(0, 5, 0, y)
...
rightArea.CanvasSize = UDim2.new(0, 0, 0, y + 60)

-- ====== 窗口拖动（修复：整个菜单可拖动） ======

local dragging = false
local dragStart, startPos
local dragObject = main  -- 拖动整个菜单

-- 任何地方都可以拖动（除了按钮）
main.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
        -- 检查是否点击了按钮
        local hit = gui:GetGuiObjectsAtPosition(i.Position.X, i.Position.Y)
        local isButton = false
        for _, obj in pairs(hit) do
            if obj:IsA("TextButton") and obj.Parent ~= top then
                isButton = true
                break
            end
        end
...
minimize.MouseButton1Click:Connect(function()
    ...
    TS:Create(main, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = minimized and UDim2.new(0, math.min(420, screenSize.X - 20), 0, 40) or UDim2.new(0, math.min(420, screenSize.X - 20), 0, math.min(520, screenSize.Y - 40))
    }):Play()
...
RS.Heartbeat:Connect(function()
    ...
    -- 穿墙
    if Toggle.Noclip then        for _, p in pairs(c:GetDescendants()) do
    ...
    if Toggle.Speed and h then
        h.WalkSpeed = 50
    elseif h and not Toggle.Speed then
    ...
    if Toggle.SuperJump and h then
        h.JumpPower = 100
    elseif h and not Toggle.SuperJump then
        h.JumpPower = Orig.JumpPower
    end
    
    -- 无限跳跃（保持状态）
    if Toggle.InfiniteJump and h then
        -- 已在上面的输入事件中处理
    end
end)
